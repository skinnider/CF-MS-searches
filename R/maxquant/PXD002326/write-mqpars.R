# Write mqpar files for the PXD002326 (Wan et al., frog) dataset.
...
for (input_file in input_files) {
  ...
  # remove experiments that were not uploaded to PRIDE
  if (expt_name == "Xl_Sucrose_2") {
    message("skipping experiment ", expt_name)
    next
  }
  ...
  # set experiments
  if (expt_name == "Xl_Sucrose_1") {
    experiments = map_chr(strsplit(fractions, "_"), 3)
  } else if (expt_name == "Xl_Sucrose_3") {
    experiments = map_chr(strsplit(fractions, "_"), 1)
  } else if (expt_name == "Xl_Sucrose_4") {
    experiments = map_chr(strsplit(fractions, "_"), 2)
  }
