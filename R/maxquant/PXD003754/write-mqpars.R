# Write mqpar files for the PXD003754 (Larance et al., MCP 2016) dataset.
...
# remove files also deposited in PXD001220
prev = read.delim("data/mqpar/PXD001220/experimentalDesignTemplate.txt") %>%
  pull(Name) %>%
  paste0('.raw')
files %<>% setdiff(prev)

# group into replicates
replicates = rep(NA, length(files))
filenames = gsub("\\..*$", "", files)
replicates = substr(filenames, 1, 8)

# assign fractions
fractions = substr(filenames, 9, nchar(filenames))

# create design data frame
design = data.frame(file = files,
                    replicate = replicates,
                    experiment = fractions) %>%
  # fix some unlabeled fractions
  group_by(replicate) %>%
  mutate(experiment = ifelse(startsWith(experiment, 'F'), experiment,
                             paste0('F', str_pad(row_number(), side = 'left', 
                                                 width = 2, pad = '0')))) %>%
  ungroup()
...
for (replicate in unique(design$replicate)) {
  ...
  for (dim_param in dim_params) {
    ...
    new_params = rep(param_line, length(experiments0))
