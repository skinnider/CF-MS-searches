# Write mqpar files for the PXD015406 (Mallam et al., Cell Rep 2019) HEK293T
# dataset.
...
# split by replicate 
replicates = gsub("_.*$", "", files)
# grab fraction names
experiments = gsub("^.*_|\\.raw$", "", files) %>%
  substr(1, 2) %>%
  paste0('F', .)
...
for (replicate in unique(replicates)) {
  ...
  experiments0 = experiments[replicates == replicate]
