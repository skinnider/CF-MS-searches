# Write mqpar files for the PXD013300 (McWhite et al., Cell 2020) 
# Triticum aestivum (1/3) dataset.
...
patts = c('wheat_germ2' = 'wgSEC1',
          'wheat_germ3' = 'wgIEX',
          'wheat_germ4' = 'wheatgerm_SEC2')
replicates = map_chr(files, ~ { 
  file = .
  names(patts)[map_lgl(patts, ~ grepl(., file))]
})
fractions = strsplit(basename(files), '_') %>%
  map2_chr(replicates, ~ extract(.x, ifelse(.y == 'wheat_germ4', 3, 2))) %>%
  gsub("a|-.*$", "", .) %>%
  paste0("F", .)
