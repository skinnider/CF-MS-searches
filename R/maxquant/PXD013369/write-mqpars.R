# Write mqpar files for the PXD013369 (McWhite et al., Cell 2020) 
# Chlamydomonas reinhardtii dataset.
...
patts = c('chlre_nuc1' = 'Chlamy_NE_SEC',
          'chlre_whole1' = 'Chlamy_SEC',
          'chlre_whole2' = 'Chlamy_WWC')
replicates = map_chr(files, ~ { 
  file = .
  names(patts)[map_lgl(patts, ~ grepl(., file))]
})
fractions = strsplit(basename(files), '_') %>%
  map2_chr(replicates, ~ extract(.x, ifelse(.y == 'chlre_nuc1', 4, 3))) %>%
  gsub("a", "", .) %>%
  paste0("F", .)
