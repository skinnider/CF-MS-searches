# Write mqpar files for the PXD013321 (McWhite et al., Cell 2020) 
# Arabidopsis thaliana (2/3) dataset.
...
replicates = ifelse(grepl("Athaliana_sproutsWWC", files), "arath_sprout1", 
                    "arath_sprout2")
fractions = strsplit(basename(files), '_') %>%
  map2_chr(replicates, ~ extract(.x, ifelse(.y == 'arath_sprout2', 5, 3))) %>%
  gsub("a|b", "", .) %>%
  paste0("F", .)
