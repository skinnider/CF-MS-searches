# Write mqpar files for PXD011304 (Pourhaighighi et al., Cell Syst 2020).
...
# read design templates
templates = list.files("data/mqpar/PXD011304", full.names = T,
                         pattern = 'experimentalDesignTemplate')
experiments = basename(templates) %>%
  gsub("_exp.*$", "", .)
for (expt_idx in seq_along(experiments)) {
  experiment = experiments[expt_idx]
  template = templates[expt_idx] %>% read.delim()
  message("processing replicate ", experiment, " ...")
  
  # limit to files in the PRIDE upload
  template %<>%
    filter(paste0(Name, '.raw') %in% basename(files))
  
  # set up fractions
  fractions = template$Name %>%
    gsub("^.*_", "", .) %>%
    gsub(" \\(.*$", "", .) %>%
    gsub("F", "", .) %>%
    str_pad(2, 'left', '0') %>%
  ...
  # create design table, and flag repeats
  design = data.frame(file = file.path(args$raw_dir,
                                       paste0(template$Name, '.raw')),
  ...
  # extract fractions
  files0 = design$file
  experiments0 = design$experiment
  fractions0 = design$fraction
  ...
  # write output
  output_filename = paste0("mqpar-", accession, "-", experiment, ".xml")
