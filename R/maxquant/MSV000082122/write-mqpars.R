# Write mqpar files for the MSV000082122 (Aryal et al., Plant Cell 2014)
# dataset.
...
# set accession
accession = "MSV000082122"

# read files
files = list.files(args$raw_dir, pattern = "*.RAW$", full.names = T) %>%
  extract(grepl("\\.RAW$", .))

# remove a single file that causes issues for MaxLFQ
files %<>% extract(!grepl("Bio2_F26", .))

# split by replicate 
replicates = gsub("_.*$", "", basename(files))
...
for (replicate in unique(replicates)) {
  ...
  # extract fractions
  experiments = gsub("\\.RAW$", "", basename(files0))
