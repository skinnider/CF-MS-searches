...
parser$add_argument('--phosphorylation', action = 'store_true', help = paste(
  'enable phosphorylation as a variable modification, and use for quant'))
parser$add_argument('--lysc', action = 'store_true', 
                    help = 'add LysC cleavage')
parser$add_argument('--no_trypsin', action = 'store_true', 
                    help = 'disable trypsin cleavage')
parser$add_argument('--disable_lfq_norm', action = 'store_true', 
                    help = 'disable LFQ normalization')
parser$add_argument('--ab_sciex', action = 'store_true', 
                    help = 'set instrument type to AB Sciex Q-TOF')
...
# mqpar: AB Sciex instruments
if (args$ab_sciex) {
  mqpar[grepl("msInstrument", mqpar)] %<>% 
    gsub(">.*<", paste0(">", 2, "<"), .)
} else {
  mqpar[grepl("msInstrument", mqpar)] %<>% 
    gsub(">.*<", paste0(">", 0, "<"), .)
...
mqpar_file = file.path(dirname(args$mqpar_file), 
                       "generated",
