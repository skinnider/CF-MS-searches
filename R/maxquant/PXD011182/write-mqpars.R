# Write mqpar files for the PXD011182 (Hu et al., Nat Meth 2019) dataset.
...
# reverse engineer experiments from MS-GF+ outputs
msgf_files = list.files('data/supplements/Hu2019/PRIDE', pattern = '^mg*', 
                        full.names = T)
expts = map(msgf_files, ~ {
  lines = readLines(.)
  mzxmls = lines[endsWith(lines, ".mzXML") & grepl("OUT=", lines)] %>%
    gsub("^.*=", "", .) %>%
    unique()
})

# cross-reference with Wan et al., 2015, and remove overlapping files
wan_summaries = list.files("data/maxquant/PXD002319", recursive = T, 
                           pattern = "summary", full.names = T) %>%
  extract(grepl("default", .))
wan_files = map(wan_summaries, read.delim) %>%
  map('Raw.file') %>%
  unlist()
expts0 = map(expts, ~ setdiff(., paste0(wan_files, '.mzXML'))) %>%
  extract(lengths(.) > 0)

# make sure all files exist
files = list.files(args$raw_dir, pattern = "*.mzXML$", full.names = T) 
filenames = basename(files)
expts1 = map(expts0, ~ intersect(., filenames))

# create design table
design = data.frame(filename = unlist(expts1),
                    replicate = paste0('expt_', rep(seq_along(expts1),
                                                    lengths(expts1)))) %>%
  # assign experiments
  mutate(experiment = map_chr(strsplit(filename, '_'), ~ 
                                extract(., startsWith(., 'P'))) %>% 
           gsub("\\..*$", "", .)) %>%
  # assign fractions
  ...
  filter(!grepl('WAN1100427_OT2_Celegans_HCW_P1D11', filename),
         !grepl('WAN110701_OT2_Celegans_HCW_P2A07', filename))
# save this file in git
write.csv(design, "data/mqpar/PXD011182/design.csv", row.names = F)
