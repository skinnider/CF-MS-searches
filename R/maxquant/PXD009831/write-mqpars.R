# Write mqpar files for the PXD009831 (Moutaoufik et al., iScience 2019)
# IEX/DLNC dataset.
...
# design:
#' PXD009831: 87 fractions = DNLCs, IEC
#' PXD009832: 95 fractions = ECSCs, IEC
#' PXD009833: 84 fractions = SEC
#' PXD009834: 84 fractions = SEC

# read files
files = list.files(args$raw_dir, pattern = "*\\.raw$", full.names = F)

# extract replicate and fractions
split = strsplit(files, '_')
fraction_names = map_chr(split, 4)
fractions = map_chr(strsplit(fraction_names, '-'), 2) %>%
  str_pad(width = 2, side = 'left', pad = '0') %>%
  paste0('F', .)
replicates = map_chr(strsplit(fraction_names, '-'), 3) %>%
  paste0('R', .)
