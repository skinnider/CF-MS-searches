# Write mqpar files for the PXD008974 (Senkler et al., Curr Biol 2018) dataset.
...
# all files are from one experiment
experiments = gsub("^.*_|\\..*$", "", basename(files)) %>%
  paste0('F', .)
...
# write output
output_filename = paste0("mqpar-", accession, "-map.xml")
