# Write mqpar files for the PXD011088 (Rugen et al., MCP 2019) dataset.
...
# assign each file to a replicate
replicates = rep(NA, length(files))
replicates[grepl("^ZK", files)] = 'Rep1'
replicates[grepl("^GP", files)] = 'Rep3'
nchars = gsub("\\..*$", "", files) %>% nchar()
replicates[nchars == 3] = 'Rep2'
replicates[nchars == 2] = 'Rep4'

# calculate fractions for each file
fractions = rep(NA, length(files))
fractions[replicates == 'Rep1'] = files[replicates == 'Rep1'] %>%
  strsplit('_') %>%
  map_chr(2) %>%
  paste0('F', .)
fractions[replicates == 'Rep2'] = files[replicates == 'Rep2'] %>%
  gsub("\\..*$", "", .) %>%
  paste0('F', .)
fractions[replicates == 'Rep3'] = files[replicates == 'Rep3'] %>%
  gsub("\\..*$", "", .) %>%
  strsplit('_') %>%
  map_chr(3) %>%
  paste0('F', .)
fractions[replicates == 'Rep4'] = files[replicates == 'Rep4'] %>%
  gsub("\\..*$", "", .) %>%
  paste0('F', .)
...
design = data.frame(file = files,
  ...
  # remove sucrose fraction 16
  filter(!(replicate == 'Rep4' & experiment == 'F16')) %>%
