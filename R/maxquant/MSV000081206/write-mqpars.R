# Write mqpar files for the MSV000081206 (Aryal et al., J Proteomics 2017)
# Arabidopsis 'update' dataset.
...
# set accession
accession = "MSV000081206"

# read files
files = list.files(args$raw_dir, pattern = "*wiff$", full.names = T)

# check that .scan files exist, too
scan_files = paste0(files, '.scan')
# remove two wiff files with scan files missing from PRIDE upload
files %<>% extract(file.exists(scan_files))

# split by replicate 
replicates = ifelse(grepl("sec2", files), "Bio2", "Bio1")

# write mqpar file for each replicate
for (replicate in unique(replicates)) {
  message("processing replicate ", replicate, " ...")
  
  # extract fractions
  files0 = files[replicates == replicate]
  experiments = gsub("\\.wiff$", "", basename(files0))
