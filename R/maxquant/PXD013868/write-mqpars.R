# Write mqpar files for the PXD013868 (Mergner et al., Nature 2020) dataset.
...
# read design table
design = read.csv("data/mqpar/PXD013868/RawfileAnnotations_Experiment.csv") %>%
  # filter to SEC
  filter(Experiment.series == 'Size exclusion chromatography') %>%
  # convert to standard format
  mutate(file = paste0(Raw.file.name, '.raw'),
         replicate = Peptide.source,
         experiment = paste0('F', str_pad(Offline.fraction, width = 2,
                                          side = 'left', pad = '0'))) %>%
