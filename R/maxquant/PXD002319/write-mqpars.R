# Write mqpar files for the PXD002319 (Wan et al., nematode) dataset. 
...
for (input_file in input_files) {
  ...
  # remove some random ones (IPs?) for one experiment
  if (expt_name == "Ce_Bead_IEX_8")
    fractions %<>% extract(!grepl("_IP_", .))
  ...
  for (dim_param in dim_params) {
    ...
    if (n_distinct(param_lines) > 1) 
      stop("manually investigate parameter: ", dim_params)
