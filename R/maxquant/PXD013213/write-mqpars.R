# Write mqpar files for the PXD013213 (McWhite et al., Cell 2020) 
...
patts = c('orysj_leaf1' = 'RiceL_IEX',
          'orysj_leaf2' = 'RiceLeaf_SEC',
          'orysj_leaf3' = 'Rice_leaf_IEX')
replicates = map_chr(files, ~ { 
  file = .
  names(patts)[map_lgl(patts, ~ grepl(., file))]
})
fractions = strsplit(basename(files), '_') %>%
  map2_chr(replicates, ~ extract(.x, ifelse(.y == 'orysj_leaf3', 5, 3))) %>%
  gsub("a|-.*$", "", .) %>%
  paste0("F", .)
