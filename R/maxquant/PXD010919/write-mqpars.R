# Write mqpar files for the PXD010919 (Gorka et al., Sci Rep 2019) dataset.
...
# extract replicates
split = strsplit(gsub("\\..*$", "", files), '_')
replicates = map_chr(split, 5) %>%
  gsub('S', '', .) %>%
  as.integer()
replicates = paste0(ifelse(replicates %% 2 == 0, 'EN_', 'ED_'), 
                    ceiling(replicates / 2))

# extract fractions
fractions = map_chr(split, 4) %>%
  gsub("Fract", "", .) %>%
