# Write mqpar files for the PXD013198 (McWhite et al., Cell 2020) 
# Glycine max dataset.
...
replicates = ifelse(grepl("SoySEC", files), "soybn_sprout2", "soybn_sprout1")
fractions = strsplit(basename(files), '_') %>%
  map2_chr(replicates, ~ extract(., ifelse(.y == 'soybn_sprout2', 3, 4))) %>%
  gsub("a", "", .) %>%
  paste0("F", .)
