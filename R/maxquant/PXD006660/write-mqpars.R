# Write mqpar files for the PXD00660 (Kastritis et al., MSB 2017) dataset.
...
files = list.files(args$raw_dir, pattern = "*.raw$", full.names = T) %>%
  extract(grepl("\\.raw$", .)) %>%
  # get co-elution only 
  extract(grepl("TB_FOR", .))
# extract replicates
replicates = strsplit(basename(files), '_') %>%
  map_chr(4)
# extract fractions
fractions = strsplit(basename(files), '_') %>%
  ...
  gsub("\\.raw$", "", .)
# create design table
design = data.frame(replicate = replicates,
...
for (replicate in unique(design$replicate)) {
  ...
  # extract fractions
  files0 = design %>% filter(replicate == !!replicate) %>% pull(file)
  experiments = design %>% filter(replicate == !!replicate) %>% pull(experiment)
  ...
  new_lines = c("            <string>DimethLys0;DimethNter0</string>",
                "            <string>DimethLys4;DimethNter4</string>")
